...
export default function TrainerChatPage() {
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch(`/api/chat?otherUserId=${userId}`);
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
      ...
      <div className="h-20 border-b border-white/10 flex items-center px-6 gap-4 bg-[#111]">
        ...
        <div>
          <h2 className="font-black text-xl">Athlete Chat</h2>
          <p className="text-xs text-white/40 uppercase tracking-widest">End-to-end Support</p>
        </div>
      </div>
